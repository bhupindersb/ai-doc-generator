...
const testimonials = [
  ...
  { name: "Rahul Gupta", role: "CTO @ DevStartup", text: "The API reference generation alone is worth the subscription. Output quality is incredible.", avatar: "RG" },
];
    ...
    <div className="min-h-screen bg-ink-900 overflow-x-hidden">
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center px-6 pt-28 pb-20">
        ...
        <div className="relative z-10 flex flex-col items-center text-center max-w-5xl mx-auto">
          ...
          >
            <Link href="/dashboard" className="bg-lime text-ink-900 font-bold px-8 py-4 rounded-xl text-sm hover:bg-lime-dim transition-all hover:scale-[1.03] lime-glow inline-flex items-center gap-2">
              Generate docs free <span className="text-base">→</span>
            </Link>
            <Link href="#demo" className="border border-ink-600 text-ink-200 font-semibold px-8 py-4 rounded-xl text-sm hover:border-lime/40 hover:text-lime transition-all inline-flex items-center gap-2">
          ...
          >
            <div className="flex items-center">
              {["#7C3AED","#C8F135","#5a5a80","#a78bfa","#F0F0F5","#ef4444"].map((c, i) => (
                <div key={i} className="w-8 h-8 rounded-full border-2 border-ink-900 relative"
            ...
            <div className="text-center sm:text-left">
              <div className="flex items-center gap-1 justify-center sm:justify-start mb-0.5">
                {[...Array(5)].map((_, i) => <span key={i} className="text-lime text-xs">★</span>)}
              </div>
        ...
        >
          <div className="bg-ink-800 border border-ink-600 rounded-2xl overflow-hidden shadow-2xl">
            <div className="flex items-center gap-2 px-5 py-3.5 border-b border-ink-600 bg-ink-900/80">
              ...
              <span className="ml-3 text-xs font-mono text-ink-500 flex-1">docs.beveez.tech — generate documentation</span>
            ...
            <div className="flex items-center gap-3 px-5 py-2.5 border-b border-ink-700 bg-ink-800/60">
              <div className="flex gap-1.5">
                {["Full Docs","README","API Ref","Inline"].map((t, i) => (
                  <span key={t} className={`text-xs font-mono px-2.5 py-1 rounded-md ${i === 0 ? "bg-lime text-ink-900 font-bold" : "text-ink-400"}`}>{t}</span>
                ))}
              </div>
              <div className="ml-auto">
                <span className="text-xs font-mono bg-ink-700 border border-ink-600 text-lime px-2.5 py-1 rounded-md">php</span>
              </div>
            </div>
            <div className="grid md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-ink-700">
              <div className="p-6">
                <p className="text-xs font-mono text-ink-500 mb-4 tracking-wider uppercase">← Input</p>
...
function getAuthUser() {
  ...
  return $db->query(
    ...
    [$_SESSION['user_id']]
              ...
              <div className="p-6">
                <p className="text-xs font-mono text-ink-500 mb-4 tracking-wider uppercase">→ Generated docs</p>
                <div className="space-y-3 text-sm font-mono">
                  <p className="text-lime font-bold text-base">## header.php</p>
                  <p className="text-ink-300 leading-relaxed">Bootstrap file for Beveez v2.1.0. Initialises the session, defines constants, and loads the authenticated user context.</p>
                  ...
                  <div className="border-t border-ink-700 pt-3">
                    ...
                    <p className="text-ink-400 text-xs mt-1">Returns the current user from DB using session, or null if unauthenticated.</p>
                    <p className="text-ink-600 text-xs mt-1">@return array|null</p>
                  </div>
                </div>
              </div>
            </div>
            <div className="px-5 py-3 border-t border-ink-700 bg-ink-900/60 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <span className="text-xs font-mono bg-ink-700 text-ink-300 px-3 py-1.5 rounded-lg">Copy</span>
                <span className="text-xs font-mono bg-ink-700 text-ink-300 px-3 py-1.5 rounded-lg">↓ .md</span>
                <span className="text-xs font-mono bg-lime text-ink-900 font-bold px-3 py-1.5 rounded-lg">Generate ✦</span>
              </div>
            </div>
          </div>
          <div className="flex flex-wrap items-center justify-center gap-6 mt-8">
            {["✓ No credit card required", "✓ Free tier available", "✓ Works with PHP, Python, JS, Go + 16 more"].map(b => (
        ...
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            ...
            <h2 className="text-4xl md:text-5xl font-display font-bold mb-5">
              Everything you need,<br /><span className="text-lime">nothing you don't</span>
            </h2>
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {features.map(f => (
              <div key={f.title} className="group bg-ink-800 border border-ink-700 rounded-2xl p-6 hover:border-lime/25 transition-all cursor-default">
                ...
                <h3 className="font-display font-bold text-base mb-2 group-hover:text-lime transition-colors">{f.title}</h3>
                <p className="text-ink-500 text-sm leading-relaxed font-mono">{f.desc}</p>
              </div>
            ))}
          </div>
          <div className="text-center mt-10">
            <Link href="/features" className="text-sm font-mono text-lime hover:underline">
              See all features →
            </Link>
          </div>
        </div>
        ...
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            ...
            <h2 className="text-4xl md:text-5xl font-display font-bold">
              Three steps to<span className="text-lime"> perfect docs</span>
            </h2>
          </div>
            ...
            {steps.map((s, i) => (
              <div key={s.n} className="relative">
                <div className="flex items-center gap-4 mb-5">
                  ...
                  {i < 2 && <div className="hidden md:block flex-1 h-px bg-gradient-to-r from-lime/20 to-transparent" />}
                </div>
      ...
      <section className="py-16 px-6 border-t border-ink-800 overflow-hidden">
        ...
        <div className="flex gap-3 flex-wrap justify-center max-w-4xl mx-auto">
          {["TypeScript","JavaScript","Python","PHP","Go","Rust","Java","C++","C#","Ruby","Swift","Kotlin","Bash","SQL","HTML/CSS","R","Scala","Elixir"].map(lang => (
            <span key={lang} className="bg-ink-800 border border-ink-700 text-ink-300 text-xs font-mono px-3 py-1.5 rounded-full hover:border-lime/30 hover:text-lime transition-all cursor-default">
              {lang}
            </span>
          ))}
        </div>
      ...
      <section className="py-28 px-6 border-t border-ink-800">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            ...
            <h2 className="text-4xl md:text-5xl font-display font-bold">Loved by developers</h2>
          </div>
          <div className="grid md:grid-cols-3 gap-5">
            {testimonials.map(t => (
              <div key={t.name} className="bg-ink-800 border border-ink-700 rounded-2xl p-7 flex flex-col">
                <div className="flex mb-5">
                  {[...Array(5)].map((_, i) => <span key={i} className="text-lime text-sm">★</span>)}
                </div>
                <p className="text-ink-200 text-sm leading-relaxed font-mono flex-1 mb-6">"{t.text}"</p>
      ...
      <section id="pricing" className="py-28 px-6 border-t border-ink-800">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            ...
            <h2 className="text-4xl md:text-5xl font-display font-bold mb-5">Simple, honest pricing</h2>
            <p className="text-ink-400 font-mono text-sm">Start free. Upgrade when you need more. Cancel anytime.</p>
          </div>
          <PricingTable />
          <p className="text-center text-xs font-mono text-ink-600 mt-8">
            All prices in USD · Payments via Razorpay · 7-day free trial on paid plans
          </p>
        </div>
      </section>
        ...
        <div className="max-w-4xl mx-auto">
          <div className="relative bg-ink-800 border border-ink-700 rounded-3xl p-14 text-center overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <h2 className="text-4xl md:text-5xl font-display font-bold mb-5">
                Start documenting in<br /><span className="text-lime">30 seconds</span>
              </h2>
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                <Link href="/dashboard" className="bg-lime text-ink-900 font-bold px-10 py-4 rounded-xl text-sm hover:bg-lime-dim transition-all hover:scale-[1.03] lime-glow">
                  Generate your first doc →
                </Link>
                <Link href="/pricing" className="border border-ink-600 text-ink-200 font-semibold px-8 py-4 rounded-xl text-sm hover:border-lime/40 hover:text-lime transition-all">
                  View pricing
                </Link>
              </div>
            </div>
          ...
          <div className="flex flex-col md:flex-row gap-12 mb-12">
            <div className="md:w-64 flex-shrink-0">
              <div className="flex items-center gap-3 mb-4">
                <Logo />
                <span className="font-display font-bold text-2xl">Doc<span className="text-lime">AI</span></span>
              </div>
          ...
          <div className="border-t border-ink-800 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex items-center gap-4 text-xs font-mono text-ink-600 flex-wrap justify-center">
              ...
              <span className="flex items-center gap-1.5">
                <span className="w-1.5 h-1.5 bg-lime rounded-full" />All systems operational
              </span>
            </div>
          </div>
        </div>
